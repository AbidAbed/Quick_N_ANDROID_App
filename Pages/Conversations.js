...
import useConfigAddContact from '../Hooks/useConfigAddContact';
import useConfigContact from '../Hooks/useConfigContact';
import NewGroupIcon from 'react-native-vector-icons/AntDesign';
import NewContactIcon from 'react-native-vector-icons/AntDesign';
...
function Conversations() {
  ...
  const [isContactsPressed, setIsContactsPressed] = useState(false);

  const [addContactResponseError, setAddContactResponseError] = useState('');
  ...
  const getUserContactsResponse = useGetUserContactsQuery({
    userId: user._id,
    token: user.token,
    isAdmin: user.isAdmin,
  });
  ...
  const [postAddUserContact, postAddUserContactResponse] =
    usePostAddUserContactMutation();

  const [patchRemoveUserContact, patchRemoveUserContactResponse] =
    usePatchRemoveUserContactMutation();
  //////////////////////// USEEFFECTS ////////////////////////////////
              ...
              return (
                <View
                  style={{
                    gap: 0,
                    borderBottomWidth:
                      userConversations.length - 1 === index ? 0 : 0.5,
                    borderColor: 'black',
                    marginTop: 14,
                    padding: 8,
                  }}>
                      ...
                      <Image
                        ...
                        source={
                          item.convAvatar
                            ? {
                                uri: item.isGroup
                                  ? `${BASE_BACKEND_URL}/message/file/binary?fileId=${item?.convAvatar}&page=1`
                                  : item.convAvatar,
                              }
                            : item.isGroup
                            ? groupPic
  ...
  useEffect(() => {
    ...
    if (path === 'Profile' || path === 'Logout' || path === 'Search') {
      navigation.getParent().setOptions({
        tabBarStyle: {
          display: 'none',
        },
  ...
  useEffect(() => {
    if (
      !getUserContactsResponse.isLoading &&
      !getUserContactsResponse.isUninitialized
    ) {
      if (getUserContactsResponse.isError) {
      } else {
        dispatch(fetchUserContacts(getUserContactsResponse.data));
      }
    }
  ...
  useEffect(() => {
    if (
      !postAddUserContactResponse.isLoading &&
      !postAddUserContactResponse.isUninitialized
    ) {
      if (postAddUserContactResponse.isError) {
        console.log(postAddUserContactResponse);
        setAddContactResponseError(postAddUserContactResponse.error.data.msg);
      } else {
        dispatch(addContact(postAddUserContactResponse.data));
        setAddContactResponseError(' ');
      }
    }
  }, [postAddUserContactResponse]);

  useEffect(() => {
    if (
      !patchRemoveUserContactResponse.isLoading &&
      !patchRemoveUserContactResponse.isUninitialized
    ) {
      if (patchRemoveUserContactResponse.isError) {
      } else {
        dispatch(
          removeContact({
            _id: patchRemoveUserContactResponse.originalArgs.userId,
          }),
        );
      }
    }
  }, [patchRemoveUserContactResponse]);
  ////////////////////// HOOKS  //////////////////////
  ...
  function handleRemoveContact(userToBeRemoved) {
    patchRemoveUserContact({
  ...
  return (
    <View
      ...
      {/** Start A Chat Modal && Contacts Modal*/}
      {isStartAChatPressed ? (
        <ModalList
          pageName="Start a new chat"
          description="My Contacts On QuickN "
          listItems={[...contacts]
              ...
              <Button
                ...
                <View
                  ...
                  <View
                    ...
                    <NewContactIcon name="adduser" color="white" size={40} />
                  </View>
                  <View style={{paddingLeft: '2%'}}>
                    <Text style={{color: 'gray', fontWeight: 'bold'}}>
                      New Contact
      ...
      {isContactsPressed ? (
        <ModalList
          ...
          listItems={[...contacts]
            .sort(useSortItems('username'))
            .filter(usr => usr._id !== user._id)}
          configHook={useConfigContact(handleRemoveContact)}
          setIsModalPressed={setIsContactsPressed}
          isModalPressed={isContactsPressed}
          isLoading={patchRemoveUserContactResponse.isLoading}
          searchKey="username"
          headerComponent={
            <View
              style={{
              ...
              <Form
                ...
                responseError={addContactResponseError}
                config={useConfigAddContact()}
                onSubmimsubmitButtonText="Save"
                submitButtonStyle={[
                  style.submitFormButton,
                  {backgroundColor: '#058095'},
                ]}
                submitButtonTextStyle={style.submitFormButtonText}
                isLoading={postAddUserContactResponse.isLoading}
                styleSingleField={{
                  flexDirection: 'row',
                  flex: 1,
                  paddingTop: '4%',
